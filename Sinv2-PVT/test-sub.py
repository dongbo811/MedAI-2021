...
if __name__ == '__main__':
    ...
    for id in range(1, 6):
        ...
        parser = argparse.ArgumentParser()
        parser.add_argument('--testsize', type=int, default=352, help='testing size')
        parser.add_argument('--pth_path', type=str, default='/home/zjudb/DATA2/' + name + '/model_pth/' + id + '_PolypPVT/PolypPVT.pth')
        opt = parser.parse_args()
        model = Network()
        model.load_state_dict(torch.load(opt.pth_path))
        model.cuda()
        ...
        for _data_name in ['KS-T1']:
            ...
            ##### put data_path here #####
            data_path = '/home/zjudb/DATA2/KS-T/'
            ##### save_path #####
            save_path = '/home/zjudb/DATA2/' + name + '/result_map/' + id + '_PolypPVT-T/masks/'
            ...
            for i in range(num1):
                ...
                res= model(image)
                h, w = im.size
                res = F.upsample(res[0] + res[1] + res[2] + res[3], size=[w,h], mode='bilinear', align_corners=False)
